...
local mod = SWDKP:NewModule("log", "AceComm-3.0")
...
local function AppendToLog(kind, event_type, name, reason, amount, mass, undo)
  if not undo then
    ...
    mod:SendCommMessage("SWDKP", string.format(LOG_FORMAT, unpack(entry)),
                        "GUILD", nil, "BULK")
...
function mod:LogSync(prefix, msg, distribution, sender)
  if prefix == "SWDKP" and sender ~= UnitName("player") then
...
local function LogRecordToString(record)
  ...
  if kind == "EP" then
    return string.format("%s: %+.2f DKP Awarded (%s) to %s",
                         date("%Y-%m-%d %H:%M", timestamp), amount, reason, name)
  elseif kind == "GP" then
    return string.format("%s: %+.2f DKP Charged (%s) to %s",
                         date("%Y-%m-%d %H:%M", timestamp), amount, reason, name)
  elseif kind == "BI" then
...
function mod:GetLogItemLink(i)
  local logsize = #self.db.profile.log
  assert(i >= 0 and i < #self.db.profile.log, "Index "..i.." is out of bounds")

  local timestamp, kind, name, reason, amount = unpack(self.db.profile.log[logsize - i])
  return reason
end
...
function mod:UndoLastAction()
  ...
  if kind == "EP" then
    SWDKP:IncEPBy(name, "Undo".." "..reason, -amount, false, true)
  elseif kind == "GP" then
    SWDKP:IncGPBy(name, "Undo".." "..reason, -amount, false, true)
  elseif kind == "BI" then
    SWDKP:BankItem("Undo".." "..reason, true)
  else
    assert(false, "Unknown record in the log")
  end
...
function mod:RedoLastUndo()
  ...
  local ep, gp, main = SWDKP:GetEPGP(name)
  if kind == "EP" then
    SWDKP:IncEPBy(name, "Redo".." "..reason, amount, false, true)
    table.insert(self.db.profile.log, record)
  elseif kind == "GP" then
    SWDKP:IncGPBy(name, "Redo".." "..reason, amount, false, true)
    table.insert(self.db.profile.log, record)
  else
    assert(false, "Unknown record in the log")
  end
...
function mod:Export()
  ...
  d.base_gp = SWDKP:GetBaseGP()
  d.min_ep = SWDKP:GetMinEP()
  d.decay_p = SWDKP:GetDecayPercent()
  d.extras_p = SWDKP:GetExtrasPercent()
  d.timestamp = GetTimestamp()

  d.roster = SWDKP:ExportRoster()
...
function mod:Import(jsonStr)
  ...
  if not success then
    SWDKP:Print("The imported data is invalid")
    return
  end
  ...
  SWDKP:Print(("Importing data snapshot taken at: %s"):format(
               date("%Y-%m-%d %H:%M", d.timestamp)))
  SWDKP:SetGlobalConfiguration(d.decay_p, d.extras_p, d.base_gp, d.min_ep)
  SWDKP:ImportRoster(d.roster, d.base_gp)
...
function mod:OnEnable()
  SWDKP.RegisterCallback(mod, "EPAward", AppendToLog, "EP")
  SWDKP.RegisterCallback(mod, "GPAward", AppendToLog, "GP")
  SWDKP.RegisterCallback(mod, "BankedItem", AppendToLog, "BI")
  mod:RegisterComm("SWDKP", "LogSync")

  -- Upgrade the logs from older dbs
  if SWDKP.db.profile.log then
    self.db.profile.log = SWDKP.db.profile.log
    SWDKP.db.profile.log = nil
  end
  if SWDKP.db.profile.redo then
    self.db.profile.redo = SWDKP.db.profile.redo
    SWDKP.db.profile.redo = nil
  end

  -- This is kept for historical reasons. See:
  -- http://code.google.com/p/epgp/issues/detail?id=350.
  SWDKP.db.RegisterCallback(self, "OnDatabaseShutdown", "Snapshot")
end
